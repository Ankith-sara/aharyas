...
const CartContextProvider = ({ children }) => {
    ...
    const syncCartToServer = useCallback(async (operation, payload) => {
        ...
        if (!userId) return;
        try { await api.post(`/api/v1/cart/${operation}`, { userId, ...payload }); } catch (err) { console.error("syncCartToServer error", err); }
    }, []);
    ...
    const getCartCount = useCallback(() =>
        Object.values(cartItems).reduce(
            (total, sizes) => total + Object.values(sizes).reduce((s, qty) => s + (qty > 0 ? qty : 0), 0), 0
        ), [cartItems]);
    ...
    const getCartItems = useCallback(() =>
        Object.entries(cartItems).flatMap(([itemId, sizes]) => {
            const product = products.find((p) => p._id === itemId);
            if (!product) return [];
            return Object.entries(sizes).filter(([, qty]) => qty > 0).map(([size, quantity]) => ({ ...product, size, quantity }));
        }), [cartItems, products]);
    ...
    const getUserCart = useCallback(async (userToken) => {
        ...
        if (!userId) return;
        try {
            const { data } = await api.post("/api/v1/cart/get", { userId }, { headers: { Authorization: `Bearer ${userToken}` } });
            ...
            if (hasGuest) {
                ...
                for (const [itemId, sizes] of Object.entries(guestCart)) {
                    ...
                    for (const [size, qty] of Object.entries(sizes)) {
                        ...
                        syncOps.push(api.post("/api/v1/cart/add", { userId, itemId, size, quantity: qty }, { headers: { Authorization: `Bearer ${userToken}` } }));
                    }
                }
    ...
    const addToWishlist = useCallback(async (itemId) => {
        ...
        try {
            ...
            if (data.success) { setWishlistItems(data.wishlist); return true; }
        } catch (error) {
    ...
    const removeFromWishlist = useCallback(async (itemId) => {
        if (!token) {
            const updated = safeRead(GUEST_WISHLIST_KEY, []).filter((id) => id !== itemId);
            safeWrite(GUEST_WISHLIST_KEY, updated);
            setWishlistItems(updated);
            return true;
        }
        try {
            ...
            if (data.success) { setWishlistItems(data.wishlist); return true; }
        } catch (err) { console.error("removeFromWishlist error", err); }
        return false;
    }, [token]);
    ...
    const toggleWishlist = useCallback(async (itemId) => {
        ...
        try {
            ...
            if (data.success) { setWishlistItems(data.wishlist); return data.isAdded; }
    ...
    const getUserWishlist = useCallback(async (userToken) => {
        try {
            const { data } = await api.post("/api/v1/wishlist/get", {}, { headers: { Authorization: `Bearer ${userToken}` } });
            if (data.success && isMounted.current) {
                const serverWishlist = data.wishlist || [];
                const guestWishlist = safeRead(GUEST_WISHLIST_KEY, []);
                const toAdd = guestWishlist.filter((id) => !serverWishlist.includes(id));
                if (toAdd.length) {
                    await Promise.allSettled(toAdd.map((itemId) =>
        ...
        } catch (err) { console.error("getUserWishlist error", err); }
    }, []);
    ...
    useEffect(() => {
        if (!token) {
            setCartItems(safeRead(GUEST_CART_KEY, {}));
            setWishlistItems(safeRead(GUEST_WISHLIST_KEY, []));
        }
    }, [token]);
    ...
    const value = useMemo(() => ({
        ...
        wishlistItems,
        appliedCoupon, applyCoupon, clearCoupon, getAppliedCoupon,
        delivery_fee: DELIVERY_FEE_TELANGANA,
        getDeliveryFee,
        ...
        cartItems, wishlistItems, appliedCoupon,
        getDeliveryFee,
        addToCart, updateQuantity, removeFromCart, clearCart,
        getCartCount, getCartAmount, getCartItems,
        applyCoupon, clearCoupon, getAppliedCoupon,
        addToWishlist, removeFromWishlist, toggleWishlist,
        isInWishlist, getWishlistCount, getWishlistProducts,
    ]);
