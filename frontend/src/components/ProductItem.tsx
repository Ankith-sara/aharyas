...
const useHoverImage = (src?: string | null) => {
  const [loaded, setLoaded] = useState(false);
  const [hovering, setHovering] = useState(false);
  const fetchedRef = useRef(false);

  useEffect(() => {
    if (!src || !hovering || fetchedRef.current) return;
    fetchedRef.current = true;
    const img = new window.Image();
    img.onload = () => setLoaded(true);
    img.src = optimizeUrl(src, 800);
  }, [hovering, src]);

  return { loaded, hovering, setHovering };
};
...
export default function ProductItem({ id, slug, image, name, price, company, discount = 0 }: ProductItemProps) {
  ...
  const primarySrc = optimizeUrl(image?.[0], 800);
  const primarySrcSet = buildSrcSet(image?.[0]);
  const secondarySrc = image?.[1] ? optimizeUrl(image[1], 800) : null;

  const { loaded: hoverLoaded, hovering, setHovering } = useHoverImage(secondarySrc);
        ...
        <div className="relative aspect-[3/4] overflow-hidden bg-gray-100">
          {!imgError ? (
            <>
              ...
              {secondarySrc && hoverLoaded && (
                <img
                  className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-500 ${
                    hovering ? 'opacity-100' : 'opacity-0'
                  }`}
                  src={secondarySrc}
                  srcSet={buildSrcSet(image[1])}
                  sizes="(max-width: 640px) 50vw, (max-width: 1024px) 33vw, 25vw"
          ...
          {/* Discount badge */}
          {hasDiscount && (
            <div className="absolute top-2 left-2 z-10 bg-red-500 text-white text-[10px] font-bold px-2 py-0.5 uppercase tracking-wider">
              {discount}% OFF
            </div>
          )}
