...
const Wishlist = () => {
  ...
  usePageMeta({ 
    title: 'Wishlist', description: 'Your saved Aharyas pieces — handcrafted products you love.' 
  });
  ...
  const hasSelectableSizes = (product) => {
    return product.sizes && product.sizes.some(size => isSizeAvailable(size));
  };
            ...
            <div className="border border-gray-200">
              ...
              <div className="grid md:grid-cols-2 min-h-[420px]">
                ...
                <div className="hidden md:flex items-center justify-center bg-gray-50 relative overflow-hidden border-r border-gray-200">
                  <div className="absolute inset-0 opacity-[0.03]" style={{backgroundImage: 'repeating-linear-gradient(45deg, #000 0px, #000 1px, transparent 1px, transparent 28px)'}} />
                                ...
                                {product.discount > 0 ? (
                                  <div className="flex items-center gap-2 flex-wrap">
                                    <span className="font-semibold text-black text-sm sm:text-base">
                                      {currency}{Math.round(product.price * (1 - product.discount / 100)).toLocaleString('en-IN')}
                                    </span>
                  ...
                  {selectedProduct.discount > 0 ? (
                    <div className="flex items-center gap-2">
                      <p className="text-base sm:text-lg font-semibold text-black">
                        {currency}{Math.round(selectedProduct.price * (1 - selectedProduct.discount / 100)).toLocaleString('en-IN')}
                      </p>
                      <p className="text-sm text-gray-400 line-through">
                        {currency}{selectedProduct.price.toLocaleString('en-IN')}
                      </p>
                    </div>
                  ) : (
                    <p className="text-base sm:text-lg font-medium text-black">{currency}{selectedProduct.price.toLocaleString('en-IN')}</p>
                  )}
                </div>
              </div>
