...
export default function SellPage() {
  ...
  return (
    <div className="min-h-screen text-black mt-16 sm:mt-20">
      <section ref={heroRef} className="py-16 px-4 sm:px-8 lg:px-20 border-b border-gray-100">
        <div className="max-w-7xl mx-auto">
          <div className={`transition-all duration-900 ${heroVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            ...
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-10">
              ...
              <a href="https://admin.aharyas.com/" target="_blank" rel="noopener noreferrer" className="flex-shrink-0">
                <button className="group inline-flex items-center gap-3 bg-black text-white px-8 py-4 text-xs font-medium uppercase tracking-widest hover:bg-gray-900 transition-all">
                  Start Selling Now
            ...
            <div className="mt-14 grid grid-cols-2 sm:grid-cols-4 gap-0 border border-gray-100">
              {[
                ...
                <div key={label} className={`px-8 py-6 border-gray-100 ${i < 3 ? 'border-r' : ''}`}>
      ...
      <section ref={stepsRef} className="py-20 px-4 sm:px-8 lg:px-20">
        <div className="max-w-7xl mx-auto">
          <div className="mb-14">
          ...
          <div className="grid gap-0 sm:grid-cols-2 lg:grid-cols-4 border border-gray-100">
            {STEPS.map(({ step, title, description, icon: Icon }, i) => (
              <div
                key={step}
                className={`group relative p-8 lg:p-10 border-gray-100 hover:bg-gray-50 transition-all ${i < 3 ? 'border-b sm:border-b-0 sm:border-r' : ''} ${stepsVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}
              >
                <span className="absolute top-6 right-6 text-6xl font-light text-black/[0.05] select-none group-hover:text-black/[0.08] transition-colors">
                  {step}
                </span>

                <div className="relative z-10">
                  <div className="w-9 h-9 border border-gray-200 flex items-center justify-center mb-6 group-hover:border-black group-hover:bg-black transition-all">
                  ...
                  <div className="w-8 h-px bg-gray-200 mb-4 group-hover:w-12 group-hover:bg-black transition-all" />
      ...
      <section ref={whyRef} className="py-20 px-4 sm:px-8 lg:px-20 border-t border-gray-100 bg-gray-50/60">
        <div className="max-w-7xl mx-auto">
          <div className="mb-14">
          ...
          <div className="grid md:grid-cols-3 gap-0 border border-gray-200">
            {WHY_SELL.map(({ number, title, description }, i) => (
              <div
                key={number}
                className={`group p-10 border-gray-200 hover:bg-white transition-all ${i < 2 ? 'border-b md:border-b-0 md:border-r' : ''} ${whyVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
              >
                <span className="text-3xl font-light text-black/15 block mb-6 group-hover:text-black/25 transition-colors">
                  {number}
                </span>
                <h3 className="text-base font-medium text-black tracking-wide mb-3">{title}</h3>
                <div className="w-8 h-px bg-gray-300 mb-4 group-hover:w-12 group-hover:bg-black transition-all" />
                <p className="text-sm text-gray-500 font-light leading-relaxed">{description}</p>
              </div>
            ))}
          </div>
        ...
        <div className="relative max-w-7xl mx-auto flex flex-col lg:flex-row items-start lg:items-center justify-between gap-10">
          ...
          <div className="flex-shrink-0 max-w-sm">
            ...
            <a href="https://admin.aharyas.com/" target="_blank" rel="noopener noreferrer">
              <button className="group inline-flex items-center gap-3 border border-white/25 text-white px-8 py-4 text-xs font-medium uppercase tracking-widest hover:bg-white hover:text-black transition-all">
                Become a Seller
                <ArrowUpRight size={13} className="group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" />
              </button>
            </a>
