        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid md:grid-cols-3 gap-0 border border-gray-100">
            {CONTACT_CARDS.map(({ icon: Icon, heading, lines, accent, accent2, href }, i) => (
              <div
                key={heading}
                className={`group p-10 flex flex-col gap-6 border-gray-100 hover:bg-gray-50 transition-colors ${i < 2 ? 'border-b md:border-b-0 md:border-r' : ''}`}
              >
                <div className="flex items-center justify-between">
                  <div className="w-10 h-10 border border-gray-200 flex items-center justify-center group-hover:border-black group-hover:bg-black transition-all">
                    <Icon size={16} className="text-gray-400 group-hover:text-white transition-colors" />
                  </div>
                  {href && (
                    <ArrowUpRight
                      size={14}
                      className="text-gray-300 opacity-0 group-hover:opacity-100 group-hover:text-black transition-all"
                    />
                  )}
                </div>
                ...
                <div>
                  ...
                  {href ? (
                    <>
                      <a href={href} className="block text-base font-medium text-black hover:underline transition-all">
                        {accent}
                      </a>
                      {accent2 && (
                        <a href={href} className="block text-base font-medium text-black hover:underline transition-all mt-1">
                          {accent2}
                        </a>
                      )}
                    </>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-start">
            <div className={`transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
              ...
              <button className="mt-10 group inline-flex items-center gap-3 bg-black text-white px-8 py-4 text-xs font-medium uppercase tracking-widest hover:bg-gray-900 transition-all">
              ...
              <div className="border border-gray-100">
                ...
                <div className="divide-y divide-gray-100">
                  {WHY_ITEMS.map(({ title, body }, i) => (
                    <div key={title} className="group px-8 py-7 flex gap-5 hover:bg-gray-50 transition-colors">
