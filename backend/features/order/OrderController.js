...
import { fetchAndMigrateUserOrders, verifyAndFinaliseRazorpayOrder } from './OrderService.js';
import { trackOrderPlaced, trackCheckoutStarted } from '../analytics/AnalyticsService.js';
...
const placeOrder = async (req, res) => {
    try {
        ...
        await markCartAsConverted(userId);
...
const hashOtp = (rawOtp) => {
    const secret = process.env.JWT_SECRET || 'aharyas_otp_secret';
...
const verifyCOD = async (req, res) => {
    try {
        ...
        res.json({ success: true, message: 'COD Order confirmed', orderId: order._id, status: order.status, payment: order.payment });
    } catch (error) {
...
// Delivery person or user submits OTP to confirm delivery
const verifyDeliveryOtp = async (req, res) => {
    try {
        ...
        const order = await orderModel.findById(orderId).select('+deliveryOtp +deliveryOtpHash +deliveryOtpExpiresAt');
        if (!order)
            return res.status(404).json({ success: false, message: 'Order not found' });
        ...
        if (!isAdmin && !isOwner)
            return res.status(403).json({ success: false, message: 'Access denied. Unauthorized to verify delivery for this order.' });

        if (order.deliveryOtpVerified)
            return res.json({ success: true, message: 'Delivery already verified' });

        // Check Lockout
        if (order.deliveryOtpLockoutUntil && order.deliveryOtpLockoutUntil > Date.now()) {
            return res.status(429).json({
                success: false,
                message: 'Too many failed OTP attempts. Temporarily locked out.'
            });
        }

        // Check Expiry
        if (order.deliveryOtpExpiresAt && order.deliveryOtpExpiresAt < Date.now()) {
            return res.status(400).json({ success: false, message: 'Delivery OTP has expired.' });
        }

        const inputHash = hashOtp(otp);
        const expectedHash = order.deliveryOtpHash || (order.deliveryOtp ? hashOtp(order.deliveryOtp) : null);
        ...
        if (!matches) {
            ...
            const updateFields = { deliveryOtpAttempts: attempts };
            if (attempts >= 5) {
                updateFields.deliveryOtpLockoutUntil = new Date(Date.now() + 15 * 60 * 1000); // 15 min lockout
                logger.warn(`Delivery OTP verification locked out for order ${orderId} after 5 failed attempts.`);
            }
