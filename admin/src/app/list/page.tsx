...
import {
  ...
  Eye, EyeOff, ArrowUpDown, CheckSquare, Square
} from 'lucide-react';
...
const EditOfferPriceRow = ({ price, discount, onDiscountChange }: { price: any; discount: number; onDiscountChange: (val: number) => void }) => {
...
const EditModal = ({ product, onSuccess, onCancel, token }: { product: any; onSuccess: (p: any) => void; onCancel: () => void; token: string | null }) => {
...
const SUBCATEGORY_ORDER = Object.fromEntries(
  Object.entries(categoryData).map(([cat, val]) => [
    cat,
    val.subCategories.filter((s: string) => s !== ''),
  ])
);
const catIdx = (p: any) => { const i = CATEGORY_ORDER.indexOf(p.category); return i === -1 ? CATEGORY_ORDER.length : i; };
const subCatIdx = (p: any) => { const s = SUBCATEGORY_ORDER[p.category] || []; const i = s.indexOf(p.subCategory); return i === -1 ? s.length : i; };
const productTs = (p: any) => {
  if (p.createdAt) return new Date(p.createdAt).getTime();
  if (p.date) return typeof p.date === 'number' ? p.date : new Date(p.date).getTime();
  return 0;
};
const productUpdateTime = (p: any) => {
  if (p.updatedAt) return new Date(p.updatedAt).getTime();
  if (p.createdAt) return new Date(p.createdAt).getTime();
  if (p.date) return typeof p.date === 'number' ? p.date : new Date(p.date).getTime();
  return 0;
};
const sortByUpdateTime = (arr: any[]) =>
...
const imageThumb = (url: string, width = 300) => {
  if (!url) return url;
...
const ProductCard = ({ item, index, onEdit, onRemove, onToggleVisibility, onSelect, isSelected }: { item: any; index: number; onEdit: (p: any) => void; onRemove: (p: any) => void; onToggleVisibility: (p: any) => void; onSelect: (id: string) => void; isSelected: boolean }) => (
  <div className={`bg-white border border-gray-200 shadow-sm overflow-hidden hover:shadow-md transition-all duration-300 relative ${item.visible === false ? 'opacity-60' : ''}`}>
    ...
    <div className="relative h-60 sm:h-80">
      <img
        ...
        onError={(e: any) => { e.target.src = '/api/placeholder/300/200'; }}
      />
      {item.visible === false && (
...
export default function ListPage() {
  ...
  const applySortOption = (arr: any[], opt: string) => {
    switch (opt) {
  ...
  const getSubCategoryOptions = () =>
    categoryData[selectedCategory]?.subCategories.filter((s: string) => s !== '') || [];
  ...
  return (
    <div className="min-h-screen bg-white px-3 sm:px-6 lg:px-8 py-6 sm:py-10">
      {deleteConfirm && mounted && ReactDOM.createPortal(
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    {getSubCategoryOptions().map((sub: string) => <option key={sub} value={sub}>{sub}</option>)}
                  </select>
                </div>
              </div>
            </div>
              ...
              <div className="text-center py-16 sm:py-20">
                ...
                {list.length === 0 ? (
                  <Link href="/add-product"
                    className="inline-flex items-center gap-2 px-6 sm:px-8 py-3 bg-black text-white text-sm uppercase tracking-wide font-light hover:bg-gray-800 transition-all duration-300">
                            ...
                            <tr key={item._id} className={`hover:bg-gray-50 ${item.visible === false ? 'opacity-60' : ''}`}>
                              ...
                              <td className="px-6 py-4 text-center">
                                <div className="flex justify-center gap-2">
                                  ...
                                  <button onClick={() => setDeleteConfirm(item)}
                                    className="p-1.5 text-gray-600 hover:text-red-600 hover:bg-red-50 border border-gray-200 hover:border-red-200 transition-all duration-300">
                                    <Trash2 size={14} />
                                  </button>
                                </div>
                              </td>
