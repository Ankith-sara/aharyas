...
const FAKE_VERCEL_ANALYTICS = {
    byDay: [
        { day: "2026-07-04", pageviews: 120, visitors: 80 },
        { day: "2026-07-05", pageviews: 150, visitors: 95 },
        { day: "2026-07-06", pageviews: 190, visitors: 110 },
        { day: "2026-07-07", pageviews: 220, visitors: 140 },
        { day: "2026-07-08", pageviews: 180, visitors: 115 },
        { day: "2026-07-09", pageviews: 240, visitors: 160 },
        { day: "2026-07-10", pageviews: 320, visitors: 210 },
    ],
    byPath: [
        { requestPath: "/", pageviews: 240, visitors: 180 },
        { requestPath: "/product/quincy-crossbody-sling-bag", pageviews: 135, visitors: 110 },
        { requestPath: "/cart", pageviews: 95, visitors: 78 },
        { requestPath: "/collection", pageviews: 80, visitors: 65 },
        { requestPath: "/checkout", pageviews: 45, visitors: 38 },
    ],
    byDevice: [
        { deviceType: "mobile", pageviews: 820 },
        { deviceType: "desktop", pageviews: 540 },
        { deviceType: "tablet", pageviews: 60 },
    ],
    byCountry: [
        { country: "IN", pageviews: 750, visitors: 510 },
        { country: "US", pageviews: 420, visitors: 280 },
        { country: "GB", pageviews: 150, visitors: 90 },
        { country: "CA", pageviews: 100, visitors: 70 },
    ],
    byReferrer: [
        { referrerHostname: "instagram.com", pageviews: 620 },
        { referrerHostname: "google.com", pageviews: 410 },
        { referrerHostname: "Direct / Organic", pageviews: 390 },
    ]
};

const AnalyticsTab = ({ adminData }) => {
    ...
    const [vercelAnalytics, setVercelAnalytics] = useState(FAKE_VERCEL_ANALYTICS);
    ...
    const fetchRecentOrders = async () => {
        if (!isTokenValid(token)) { logout(); return; }
        try {
    ...
    const chartOptions = {
        ...
        scales: {
            ...
            y: { grid: { color: "rgba(0,0,0,0.05)" }, ticks: { color: "#666", font: { size: 10 }, callback: v => `₹${v.toLocaleString("en-IN")}` } },
        },
    };
    ...
    // Fake Demo Data for Daily Logins
    const dailyLoginsData = [
        { _id: { day: 3, month: 7, year: 2026 }, count: 15 },
        { _id: { day: 4, month: 7, year: 2026 }, count: 18 },
        { _id: { day: 5, month: 7, year: 2026 }, count: 12 },
        { _id: { day: 6, month: 7, year: 2026 }, count: 22 },
        { _id: { day: 7, month: 7, year: 2026 }, count: 28 },
        { _id: { day: 8, month: 7, year: 2026 }, count: 20 },
        { _id: { day: 9, month: 7, year: 2026 }, count: 23 },
    ];
    const loginsChart = {
        labels: dailyLoginsData.map(d => `${d._id.day} ${MONTH_NAMES[d._id.month - 1]}`),
        datasets: [{
            label: "Active Logins", data: dailyLoginsData.map(d => d.count),
            borderColor: "#000", backgroundColor: "rgba(0,0,0,0.05)", fill: true, tension: 0.4,
            pointBackgroundColor: "#000", pointBorderColor: "#fff", pointBorderWidth: 2, pointRadius: 3,
        }],
    };
    ...
    const mostClickedChart = {
        labels: mostClickedProducts.map(p => p.name.length > 20 ? p.name.substring(0, 20) + "..." : p.name),
        datasets: [{
            label: "Views",
            data: mostClickedProducts.map(p => p.viewCount || 0),
    ...
    const mostClickedChartOptions = {
        ...
        plugins: {
            ...
            tooltip: {
                backgroundColor: "#000", titleColor: "#fff", bodyColor: "#fff",
                borderColor: "#333", borderWidth: 1,
                callbacks: { label: ctx => ` Views: ${ctx.parsed.x}` },
            },
        },
        scales: {
    ...
    const detailedReport = (() => {
        ...
        // 2. Today's Checkout Conversion Rate
        ...
        const totalCatCount = categoryBreakdown.reduce((sum, c) => sum + c.count, 0);
        ...
        // 5. High Click, Low Conversion (Fake Demo Data)
        const highClickLowSales = [
            { name: "QUINCY – Crossbody Sling Bag", views: 135, sold: 2 },
            { name: "Indy - Dress", views: 98, sold: 1 },
            { name: "Lilibeth - dress", views: 75, sold: 0 },
            { name: "Aiko - Halter Neck Dress", views: 64, sold: 1 },
            { name: "Smitā - Pure Mulmul Cotton Saree", views: 58, sold: 0 },
            { name: "Sanmoh - men's organic indigo block prined linen and bamboo half sleeve shirt", views: 44, sold: 0 },
            { name: "Cleo - 2 piece co-ord set", views: 40, sold: 0 },
            { name: "Soleil - 2 piece co-ord set", views: 38, sold: 0 },
            { name: "Confetti - Halter neck Short top", views: 32, sold: 0 },
            { name: "Embargo - kali dress", views: 29, sold: 0 }
        ];
    ...
    // Funnel data from live counters (Fake Demo Data)
    const funnelStages = [
        { label: "Product Views", value: 43, color: "#000" },
        { label: "Added to Cart", value: 19, color: "#333" },
        { label: "Reached Checkout", value: 12, color: "#555" },
        { label: "Completed Purchase", value: 6, color: "#777" },
    ];
    ...
    return (
        <div className="max-w-8xl mx-auto space-y-4 sm:space-y-6 px-4 py-6">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                        ...
                        <div className="bg-white border border-gray-200 p-6 shadow-sm lg:col-span-6 space-y-6">
                            ...
                            <div>
                                ...
                                {detailedReport?.highClickLowSales.length > 0 ? (
                                    <div className="space-y-3 max-h-48 overflow-y-auto pr-1">
                                        {detailedReport.highClickLowSales.map((item, idx) => {
            ...
            {dashboardTab === "behavior" && (
                <div className="space-y-6">
                    {/* User Analytics Tab Content (Catalog & Engagement cards) */}
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                        {[
                            { label: "Total Products", value: 148, icon: <Package size={16} />, sub: `Views today: 342 live` },
                            { label: "Registered Users", value: 867, icon: <User size={16} />, sub: "in database" },
                            { label: "Today's Logins", value: 23, icon: <Activity size={16} />, sub: "user sessions" },
                            { label: "Cart Adds Today", value: 19, icon: <ShoppingCart size={16} />, sub: "live updates" },
                            { label: "Live Searches Today", value: 54, icon: <Search size={16} />, sub: "user queries" }
                        ].map((item, i) => (
                        ...
                        <div className="space-y-6">
                                ...
                                {[
                                    {
                                        ...
                                        value: vercelAnalytics.byDay?.reduce((sum, d) => sum + (d.pageviews || 0), 0) || 0,
                                        icon: <Eye size={16} />,
                                        sub: "Last 7 days"
                                    },
                                    {
                                        label: "Unique Visitors",
                                        value: vercelAnalytics.byDay?.reduce((sum, d) => sum + (d.visitors || 0), 0) || 0,
                                        icon: <User size={16} />,
                                        sub: "Last 7 days"
                                    },
                                    {
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                        ...
                                        <Line
                                            data={{
                                                ...
                                                datasets: [
                                                    {
                                                        label: "Page Views",
                                                        data: vercelAnalytics.byDay.map(d => d.pageviews || 0),
                                                        borderColor: "#000",
                                                    ...
                                                    {
                                                        label: "Unique Visitors",
                                                        data: vercelAnalytics.byDay.map(d => d.visitors || 0),
                                                        borderColor: "#3B82F6",
                                ...
                                <div className="bg-white border border-gray-200 p-5 shadow-sm">
                                    ...
                                    <div className="h-64 relative flex items-center justify-center">
                                        {vercelAnalytics.byDevice?.length === 0 ? (
                                            <div className="text-xs text-gray-400 italic">No device data.</div>
                                        ) : (
                                            <div className="w-56 h-56">
                                                <Doughnut
                                                    data={{
                                                        labels: vercelAnalytics.byDevice.map(d => d.deviceType || d.device || "Unknown"),
                                                        datasets: [{
                                                            data: vercelAnalytics.byDevice.map(d => d.pageviews || 0),
                                ...
                                <div className="bg-white border border-gray-200 p-5 shadow-sm">
                                    ...
                                    {vercelAnalytics.byCountry?.length > 0 ? (
                                        <div className="space-y-3">
                                            {vercelAnalytics.byCountry.map((item, idx) => {
                                ...
                                <div className="bg-white border border-gray-200 p-5 shadow-sm">
                                    ...
                                    {vercelAnalytics.byReferrer?.length > 0 ? (
                                        <div className="space-y-3">
                                            {vercelAnalytics.byReferrer.map((item, idx) => {
                                ...
                                <div className="bg-white border border-gray-200 shadow-sm overflow-hidden col-span-1 lg:col-span-2">
                                    ...
                                    <div className="overflow-x-auto">
                                        <table className="w-full text-left text-xs border-collapse font-sans">
                                            ...
                                            <tbody className="divide-y divide-gray-100">
                                                {vercelAnalytics.byPath?.map((item, idx) => {
...
const ProfileTab = ({ token, adminData, setAdminData }) => {
...
const AdminPanel = () => {
    ...
    const activeTab = location.pathname === "/profile" ? "profile" : "dashboard";
    ...
    return (
        <div className="min-h-screen bg-white">
                ...
                <div className="px-4 sm:px-6 py-3 sm:py-4">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                            ...
                            <div className="min-w-0">
                                <h1 className="text-sm sm:text-xl font-medium text-black uppercase tracking-wide leading-none">Admin Dashboard</h1>
                                <p className="text-[10px] sm:text-xs text-gray-500 uppercase tracking-wider font-light mt-0.5 hidden sm:block">Management Panel</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <main className="px-3 sm:px-6 lg:px-8 py-5 sm:py-10">
                {activeTab === "dashboard" && (
                    <AnalyticsTab adminData={adminData} />
                )}
                {activeTab === "profile" && (
                    <ProfileTab token={token} adminData={adminData} setAdminData={setAdminData} />
                )}
            </main>
        </div>
    );
