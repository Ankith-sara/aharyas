...
import {
  ShoppingBag, User, MapPin, CreditCard, Package2, Search,
...
const AddOrder = () => {
  ...
  // Filter products based on search term and sort by relevance (prefix matches first, name matches over brand matches)
  const filteredProducts = products
    ...
    .map(p => {
      ...
      if (nameIndex > -1) {
        // Name matches are prioritized. A match at the start of the name gets the lowest score (highest priority).
        score = nameIndex;
      } else if (compIndex > -1) {
        // Brand/company matches are secondary.
        score = 100 + compIndex;
      }
  ...
  const handleAddItem = () => {
    ...
    // Check if item already added with same size
